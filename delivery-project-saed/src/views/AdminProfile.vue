...
// دالة مساعدة لتحديث الـ form من object المستخدم
// غير fillForm هيك
const fillForm = (user) => {
  ...
  form.value.gender = user.gender !== null && user.gender !== undefined 
    ? Number(user.gender) : null

  // ✅ أضف http://localhost:8000 قبل /storage/
  if (user.image_path) {
    avatarPreview.value = `http://localhost:8000/storage/${user.image_path}?t=${Date.now()}`
  }
}
...
onMounted(async () => {
  try {
    ...
    fillForm(res.data.user || res.data)
  } catch (err) {
...
const saveProfile = async () => {
  ...
  try {
    ...
    if (response.data.status) {
      ...
      // ✅ حدّث الـ form مباشرة بدون reload
      ...
      // ✅ امسح الصورة المرفوعة لأنها اتحفظت

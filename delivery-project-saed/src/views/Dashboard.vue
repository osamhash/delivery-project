          ...
          <table class="dashboard-table">
            ...
            <tbody>
              <tr v-for="order in filteredOrders" :key="order.id">
                ...
                <td>
                  <button
                    v-if="!order.review && order.status?.name === 'completed'"
                    class="rate-btn"
                    @click="openReview(order)"
                  >⭐ قيّم</button>
                  <span v-else-if="order.review" class="rated">
                    ⭐ {{ order.review.rating }}
                  </span>
    ...
    <transition name="fade">
      <div v-if="showReview" class="review-overlay" @click.self="closeReview">
        <div class="review-modal">
          ...
          <h2>تقييم الطلب #{{ selectedOrder?.id }}</h2>
          ...
          <div class="review-section">
            ...
            <textarea v-model="comment" placeholder="تعليق على المتجر (اختياري)..." class="review-textarea"></textarea>
          </div>

          <!-- تقييم السائق -->
          <div class="review-section" v-if="selectedOrder?.driver">
            <p class="review-section-title">🚚 تقييم السائق</p>
            <div class="driver-info-mini" v-if="selectedOrder.driver?.user">
              <span>{{ selectedOrder.driver.user.first_name }} {{ selectedOrder.driver.user.last_name }}</span>
            </div>
            <div class="stars">
              <span
                v-for="i in 5" :key="`driver-${i}`"
                class="star"
                :class="{ active: i <= hoverDriverRating || i <= driverRating }"
                @mouseover="hoverDriverRating = i"
                @mouseleave="hoverDriverRating = 0"
                @click="driverRating = i"
              >★</span>
            </div>
            <textarea v-model="driverComment" placeholder="تعليق على السائق (اختياري)..." class="review-textarea"></textarea>
          </div>
...
const openReview = async (order) => {
  // جلب تفاصيل الطلب بما فيها السائق
  try {
...
const submitReview = async () => {
  ...
  try {
    await api.post('/reviews', {
      ...
      driver_comment: selectedOrder.value.driver ? driverComment.value : null,
    })

    // تحديث الطلب في الـ list بدون reload
    const idx = orders.value.findIndex(o => o.id === selectedOrder.value.id)
    if (idx !== -1) {
      orders.value[idx].review = { rating: rating.value, comment: comment.value }
    }
...
const toggleTheme = () => { isDark.value = !isDark.value; localStorage.setItem('delivro_theme', isDark.value ? 'dark' : 'light') }
...
.review-modal { background: #0f172a; border-radius: 24px; padding: 32px; width: 100%; max-width: 480px; border: 1px solid rgba(56,189,248,.25); position: relative; max-height: 90vh; overflow-y: auto; }
.close-review { position: absolute; top: 16px; left: 16px; background: none; border: none; color: #64748b; font-size: 20px; cursor: pointer; }
.review-modal h2 { color: #fff; margin: 0 0 24px; font-size: 20px; text-align: center; }
.review-section { margin-bottom: 24px; padding: 16px; background: rgba(255,255,255,.04); border-radius: 16px; border: 1px solid rgba(255,255,255,.06); }
.review-section-title { color: #94a3b8; font-size: 13px; font-weight: 600; margin: 0 0 12px; }
.driver-info-mini { color: #38bdf8; font-size: 13px; margin-bottom: 10px; }
...
.btn.submit { background: linear-gradient(135deg, #38bdf8, #0ea5e9); color: #0f172a; }

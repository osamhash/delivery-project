...
async function handleRegister() {
  ...
  isLoading.value = true; serverError.value = ''
  try {
    ...
    if (form.value.gender !== null) fd.append('gender',       form.value.gender)
    ...
    const map = { customer: '/customer/dashboard', provider: '/provider/dashboard', driver: '/driver/dashboard' }
    router.push(map[form.value.role] ?? '/')
  } catch (err) {
    ...
    if (data?.errors) {
      ...
      else if (mapped.first_name || mapped.last_name || mapped.provider_type) currentStep.value = 1
    } else {
      serverError.value = data?.message ?? 'حدث خطأ، يرجى المحاولة لاحقاً'
    }

          ...
          <div class="panel-header">
            ...
            <div class="filters-block">
              ...
              <select v-model="orderStatusFilter" class="table-filter">
                ...
                <option value="OnTheWay">في الطريق للتوصيل</option>
                  ...
                  <td>
                    ...
                    <div v-else class="table-actions">
                      <select 
                        ...
                        @change="changeStatusByAdmin(order, $event.target.value)" 
                        :value="order.status?.name"
                      >
                        <option value="Pending">قيد الانتظار</option>
                        <option value="Accepted">قيد التحضير</option>
                        <option value="OnTheWay">في الطريق</option>
                        <option value="Completed">مكتمل</option>
                        <option value="Rejected">مرفوض</option>
                      </select>
                      <button class="action-btn delete" @click="cancelOrderByAdmin(order.id)">
                        ❌ إلغاء
                      </button>
                    </div>
                  </td>
...
const loadAdminData = async () => {
  try {
    ...
    try {
      ...
      if (Array.isArray(drivRes.data)) {
        drivers.value = drivRes.data
      } else if (drivRes.data && drivRes.data.data) {
    ...
    const totalUsers = customers.value.length + providers.value.length + drivers.value.length
    if (totalUsers > 0) {
      successMsg.value = `تم تحميل البيانات بنجاح! (${customers.value.length} زبون، ${providers.value.length} متجر، ${drivers.value.length} سائق)`
      setTimeout(() => { successMsg.value = '' }, 3000)
    } else {
      errorMsg.value = 'لا توجد بيانات. تأكد من اتصال الخادم.'
    }
...
const mapStatusToId = (status) => {
  const map = { Pending: 1, Accepted: 2, OnTheWay: 3, Completed: 4, Rejected: 5 }
  return map[status] ?? 1
}
</script>